...
/**
 * Registers the standard TermLoop MCP tools against any TermLoopClient — real HTTP or an
 * in-process implementation.
 *
 * This is a local copy of `@termloop/client`'s `registerTermLoopTools` (packages/client-sdk/src/mcpTools.ts).
 * packages/server is compiled with plain tsc and file-copied (not bundled) into the published
 * `termloop` npm package, so a runtime import of a workspace-only package like `@termloop/client`
 * would 404 for real npm installs — keep this in sync with the client-sdk version by hand rather
 * than importing it.
 */
