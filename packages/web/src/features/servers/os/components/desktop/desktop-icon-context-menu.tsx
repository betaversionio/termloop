...
export function DesktopIconContextMenu({
  ...
  const items: MenuItem[] = [
    ...
    { kind: "separator" },
    ...(isPinnedToDock ? [] : [{ kind: "action", label: "Keep in Dock", onClick: onPinToDock } as MenuItem]),
    { kind: "action", label: "Remove from Desktop", onClick: onRemoveFromDesktop },
    ...(canUninstall
      ? [{ kind: "separator" } as MenuItem, { kind: "action", label: "Uninstall", onClick: onUninstall, destructive: true } as MenuItem]
      : []),
  ];
