...
`localScript` is an alternative to `command` for a `.sh` file that only exists locally (in your workspace), not on the remote server yet — the path is relative to the workspace folder. Selecting it via `TermLoop: Insert Command Alias` uploads its contents to a temp path on the remote server, runs it, then deletes it. It supports the same per-connection object shape as `command`. Since uploading is asynchronous, `localScript` aliases can only be run via the Command Palette picker — typing `/deploy` directly in the terminal shows a message pointing you at the picker instead, rather than sending the literal text.
